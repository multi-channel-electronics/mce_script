...
#########################################################
    ...
    subplot(311)
    #plot(data[:,idx])
    plot(unwrap(data[:,SOURCE_ROW],2**16))
    title('Feedback; data_mode 10, unwrap 2^16')#25-1+3-11=16
    ylabel('FB (DAC units)')

    subplot(312)
    plot(data_fb1[:,SOURCE_ROW])
    hold(1)
    plot(unwrap(data_fb1[:,SOURCE_ROW],2**19))#32-1-12=19
    title('Feedback; data_mode 1, unwrap 2^19')#; non-source rows')
    ylabel('FB - FB[t=0] (DAC units)')

    if data_mode == 10:
       subplot(313)
       plot(fj[:,SOURCE_ROW])#-128)
       title('flux-jump counter')
       xlabel('Time step')
       ylabel('FJ - FJ[t=0] ')
    show()
