...
; 	spawn,config_mce_file + ' >> '+todays_folder+c_filename+'.log',exit_status=status14
	...
	common ramp_sq1_var, new_adc_offset, squid_p2p, squid_lockrange, squid_lockslope, squid_multilock
        ...
        for j=0,n_elements(mask_list)-1 do $
          mask_files[j] = getenv('MAS_CONFIG') + strcompress('dead_lists/'+exp_config.array_id+ $
...
auto_setup_mask_dead,mask,filespec = getenv('MAS_CONFIG')+strcompress('dead_lists/'+exp_config.array_id+'/dead_*.cfg',/remove_all)
